...
int main() {
    ...
    register uint32_t increment asm("t2") = 8; // force it into a register to use add instead of addi
    ...
    asm volatile(
        ...
        "frep.o %[n_frep], 3, 0, 0\n"
        "add    %[res], %[res], %[inc]\n"
        // The CSR instruction is not supported by FREP and we fall back into HW loop mode.
        "csrr	t0, mhartid\n"
        "nop"
        // outputs
        : [res]"+r"(res)
