...
static inline void gemm_fp64_vec_naive_2sregs(uint32_t setup_ssr, uint32_t partition_banks,
    ...
    while (computed_col < N) {
        ...
        for(int i = 0; i < M; i+=2) {
            double *C_l2_1 = C_l1 + lda*i;
            double *C_l2_2 = C_l1 + lda*(i+1);
    ...
    while (computed_col < N) {
        ...
        for(int i = 0; i < M; i+=2) {
            ...
            if (K) {
                ...
                asm volatile(
                    // Seed ft0/ft1 with initial scalars t0/t1
                    "fmv.d   ft0, %[t0]\n"
                    "fmv.d   ft1, %[t1]\n"
                    // Repeat K iterations (frep iterates n+1 times -> pass K-1)
                    "frep.o  %[n_frep], 8, 0, 0 \n"
                    // Body:
                    "vle64.v v16, (%[ptr_b])\n"
                    "add     %[ptr_b], %[ptr_b], %[inc_b]\n"
