...
inline unsigned szrt_nof_alus() {
    return szrt_frep_state() & FREP_NOF_FU_MASK;
}
...
inline unsigned szrt_nof_lsu_slots() {
    return (szrt_frep_state() >> (1 * FREP_FU_OFFSET + FREP_NOF_SLOTS_OFFSET)) & FREP_NOF_SLOTS_MASK;
}
...
inline unsigned szrt_nof_fpu_slots() {
    return (szrt_frep_state() >> (2 * FREP_FU_OFFSET + FREP_NOF_SLOTS_OFFSET)) & FREP_NOF_SLOTS_MASK;
}
...
inline frep_mem_consistency_e szrt_frep_mem_consistency() {
    return (frep_mem_consistency_e)((szrt_frep_config() >> FREP_MEM_CONS_OFFSET) & FREP_MEM_CONS_MASK);
}
