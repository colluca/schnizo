...
// superscalar FREP mechanism.

#include "szrt.h"

static inline void vexpf_frep(double *a, double *b) {
    uint64_t ki, t;
    ...
    if (snrt_cluster_core_idx() == 0) {
        ...
        asm volatile (
            ...
            : [ InvLn2N ] "f"(InvLn2N), [ inc ] "i"(sizeof(double)), [ n_reps ] "r"(LEN-1),
              [ SHIFT ] "f"(SHIFT), [ C0 ] "f"(C[0]), [ C1 ] "f"(C[1]),
              [ C2 ] "f"(C[2]), [ C3 ] "f"(C[3]), [ ki ] "r"(&ki),
              [ t ] "r"(&t), [ T ] "r"(T)
            : "memory", "a0", "a1", "a2", "fa0", "fa1", "fa2", "fa3", "fa4", "ft0", "ft1"
        );
    }
}
