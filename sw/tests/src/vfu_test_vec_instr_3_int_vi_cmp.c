...
#define READ_MASK(dst) asm volatile("vmv.x.s %0, v0" : "=r"(dst))
...
int main() {
    if (snrt_global_core_idx() == 0) {
        ...
        // asm volatile("vle64.v v0, (%0)" :: "r"(ca));
        // asm volatile("vle64.v v4, (%0)" :: "r"(cb));
        // asm volatile("vmseq.vv v0, v0, v4");
        // READ_MASK(cmask);
        // if ((cmask & 0xF) != 0x6) { printf("vmseq.vv error: got 0x%lx\n", cmask & 0xF); errors++; }

        // asm volatile("vle64.v v0, (%0)" :: "r"(ca));
        // asm volatile("vle64.v v4, (%0)" :: "r"(cb));
        // asm volatile("vmsne.vv v0, v0, v4");
        // READ_MASK(cmask);
        // if ((cmask & 0xF) != 0x9) { printf("vmsne.vv error: got 0x%lx\n", cmask & 0xF); errors++; }

        // asm volatile("vle64.v v0, (%0)" :: "r"(ca));
        // asm volatile("vle64.v v4, (%0)" :: "r"(cb));
        // asm volatile("vmslt.vv v0, v0, v4");
        // READ_MASK(cmask);
        // if ((cmask & 0xF) != 0x1) { printf("vmslt.vv error: got 0x%lx\n", cmask & 0xF); errors++; }

        // asm volatile("vle64.v v0, (%0)" :: "r"(cau));
        // asm volatile("vle64.v v4, (%0)" :: "r"(cbu));
        // asm volatile("vmsltu.vv v0, v0, v4");
        // READ_MASK(cmask);
        // if ((cmask & 0xF) != 0x1) { printf("vmsltu.vv error: got 0x%lx\n", cmask & 0xF); errors++; }

        // asm volatile("vle64.v v0, (%0)" :: "r"(ca));
        // asm volatile("vle64.v v4, (%0)" :: "r"(cb));
        // asm volatile("vmsle.vv v0, v0, v4");
        // READ_MASK(cmask);
        // if ((cmask & 0xF) != 0x7) { printf("vmsle.vv error: got 0x%lx\n", cmask & 0xF); errors++; }

        // asm volatile("vle64.v v0, (%0)" :: "r"(cau));
        // asm volatile("vle64.v v4, (%0)" :: "r"(cbu));
        // asm volatile("vmsleu.vv v0, v0, v4");
        // READ_MASK(cmask);
