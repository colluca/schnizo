...
int main() {
    if (snrt_global_core_idx() == 0) {
        ...
        asm volatile("vse64.v v8, (%0)" ::"r"(zi));
        for (int i = 0; i < N; i++)
            if ((uint64_t)zi[i] != (xu[i] >> 2)) {
                printf("vsrl.vi error at %d\n", i);
                errors++;
            }
