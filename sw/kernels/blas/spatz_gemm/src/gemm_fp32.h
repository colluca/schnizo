...
static inline void  gemm_fp32_vec_frep_opt(
    ...
    while (computed_col < N) {
        ...
        float* C_l2_1 = C_l1;
        float* C_l2_2 = C_l2_1 + ldc;
        float* C_l2_3 = C_l2_2 + ldc;
        float* A_l2_1 = (float*)A_p;
        float* A_l2_2 = A_l2_1 + lda;
        float* A_l2_3 = A_l2_2 + lda;
        ...
        for (int i = 0; i+2 < M; i += 3) {
            ...
            if (beta) {
                asm volatile("vle32.v v0, (%0);" ::"r"(C_l2_1));
                asm volatile("vle32.v v8, (%0);" ::"r"(C_l2_2));
                asm volatile("vle32.v v16, (%0);" ::"r"(C_l2_3));
            } else {
            ...
            asm volatile(
                ...
                "flw     %[ft2], 0(%[ptr_a2]) \n"
                : [ptr_b] "+r"(B_l3_1), [ptr_a0] "+r"(A_l3_1),
                  [ptr_a1] "+r"(A_l3_2), [ptr_a2] "+r"(A_l3_3), [ft0] "+f"(t0), [ft1] "+f"(t1), [ft2] "+f"(t2)
                :
                [inc_b] "r"(inc_b), [inc_a] "r"(inc_a), [n_frep] "r"(beta ? K - 1 : K - 2)
                :);
            ...
            asm volatile("vse32.v v0, (%0);" ::"r"(C_l2_1) : "memory");
            asm volatile("vse32.v v8, (%0);" ::"r"(C_l2_2) : "memory");
            asm volatile("vse32.v v16, (%0);" ::"r"(C_l2_3) : "memory");
