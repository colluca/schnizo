...
#define SETVLEN(rd, rs)                                          \
    asm volatile("vsetvli %[vl], %[rvl], e64, m" LMUL ", ta, ma" \
                 : [ vl ] "=r"(rd)                               \
                 : [ rvl ] "r"(rs));
...
static inline void gemm_fp64_opt_4FPUs(uint32_t setup_ssr,
                                       uint32_t partition_banks,
                                       uint32_t transa, uint32_t transb,
                                       uint32_t M, uint32_t N, uint32_t K,
                                       void* A_p, uint32_t lda, void* B_p,
                                       uint32_t ldb, uint32_t beta, void* C_p,
                                       uint32_t ldc) {
    double* A = (double*)A_p;
    double* B = (double*)B_p;
    double* C = (double*)C_p;
    ...
    for (uint32_t m = 0; m < M; m += 4) {
        ...
        for (uint32_t n0 = 0; n0 < N / unroll; n0++) {
            ...
            asm volatile(
                ...
                "add     %[ptr_b], %[ptr_b], %[inc_b]\n"
                : [ c00 ] "+f"(c[0]), [ c01 ] "+f"(c[1]), [ c02 ] "+f"(c[2]),
                  [ c03 ] "+f"(c[3]), [ c10 ] "+f"(c[4]), [ c11 ] "+f"(c[5]),
                  [ c12 ] "+f"(c[6]), [ c13 ] "+f"(c[7]), [ c20 ] "+f"(c[8]),
                  [ c21 ] "+f"(c[9]), [ c22 ] "+f"(c[10]), [ c23 ] "+f"(c[11]),
                  [ c30 ] "+f"(c[12]), [ c31 ] "+f"(c[13]), [ c32 ] "+f"(c[14]),
                  [ c33 ] "+f"(c[15]), [ ptr_a0 ] "+r"(ptr_a0),
                  [ ptr_a1 ] "+r"(ptr_a1), [ ptr_a2 ] "+r"(ptr_a2),
                  [ ptr_a3 ] "+r"(ptr_a3), [ ptr_b ] "+r"(ptr_b)
                : [ inc_a ] "r"(inc_a), [ inc_b ] "r"(inc_b),
                  [ n_frep ] "r"(K - 1)
...
static inline void gemm_fp64_vec_dummy(uint32_t setup_ssr,
                                       uint32_t partition_banks,
                                       uint32_t transa, uint32_t transb,
                                       uint32_t M, uint32_t N, uint32_t K,
                                       void* A_p, uint32_t lda, void* B_p,
...
static inline void gemm_fp64_vec_dummy_scalar(
    ...
    while (computed_col < N) {
        ...
        for (int i = 0; i + 1 < M; i += 2) {
            ...
            asm volatile("dummy_scalar_loop%=: \n"
                         : /* no outputs */
                         : /* no inputs */
                         : "memory");
...
// edge cases
static inline void gemm_fp64_vec_frep(uint32_t setup_ssr,
...
// Should work for all MNK possible dimensions, not tested for very specific
// edge cases
static inline void gemm_fp64_vec_frep_test(uint32_t setup_ssr,
                                           uint32_t partition_banks,
                                           uint32_t transa, uint32_t transb,
                                           uint32_t M, uint32_t N, uint32_t K,
                                           void* A_p, uint32_t lda, void* B_p,
                                           uint32_t ldb, uint32_t beta,
                                           void* C_p, uint32_t ldc) {
    if (transa || transb || partition_banks)
        return;  // Fallback not implemented here.
...
// Needs at least K = 2
// The pointer are called according to which loop they are updated in
static inline void gemm_fp64_vec_base_unrolled(
    uint32_t setup_ssr, uint32_t partition_banks, uint32_t transa,
    uint32_t transb, uint32_t M, uint32_t N, uint32_t K, void* A_p,
    uint32_t lda, void* B_p, uint32_t ldb, uint32_t beta, void* C_p,
    uint32_t ldc) {
...
// The pointer are called according to which loop they are updated in
static inline void gemm_fp64_vec_base(uint32_t setup_ssr,
                                      uint32_t partition_banks, uint32_t transa,
                                      uint32_t transb, uint32_t M, uint32_t N,
                                      uint32_t K, void* A_p, uint32_t lda,
                                      void* B_p, uint32_t ldb, uint32_t beta,
