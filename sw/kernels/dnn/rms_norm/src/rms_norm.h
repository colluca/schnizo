...
static inline void rms_norm_fp32_naive(float *ifmap, float *weight,
                                       float *ofmap, uint32_t batch_size,
    ...
    for (uint32_t b = 0; b < batch_size; b++) {
        for (uint32_t s = core_idx; s < seq_len; s += num_cores) {
            ...
            float sum_sq = 0.0f;
            for (uint32_t d = 0; d < hidden_dim; d++) sum_sq += x[d] * x[d];
...
static inline void rms_norm_fp32_baseline(float *ifmap, float *weight,
                                          float *ofmap, uint32_t batch_size,
                                          uint32_t seq_len, uint32_t hidden_dim,
                                          float eps) {
    uint32_t num_cores = snrt_cluster_compute_core_num();
    ...
    for (uint32_t b = 0; b < batch_size; b++) {
        for (uint32_t s = core_idx; s < seq_len; s += num_cores) {
            ...
            float inv_rms =
                1.0f / sqrtf((sq0 + sq1 + sq2 + sq3) / hidden_dim + eps);
...
static inline void rms_norm_fp32_schnizo(float *ifmap, float *weight,
                                         float *ofmap, uint32_t batch_size,
                                         uint32_t seq_len, uint32_t hidden_dim,
                                         float eps) {
    uint32_t num_cores = snrt_cluster_compute_core_num();
...
// Pass 1: 4-stream fmadd.s reduction (9 insns). Pass 2: dual fmul.s 8-unrolled (43 insns).
// Assumes hidden_dim % 8 == 0.
static inline void rms_norm_fp32_schnova(float *ifmap, float *weight,
                                         float *ofmap, uint32_t batch_size,
                                         uint32_t seq_len, uint32_t hidden_dim,
                                         float eps) {
    uint32_t num_cores = snrt_cluster_compute_core_num();
...
static inline void rms_norm_layer(rms_norm_layer_t l) {
    ...
    uint32_t tile_size =
        l.batch_size * tile_seq_len * l.hidden_dim * data_type_size;
    ...
    for (uint32_t cluster_tile_idx = 0; cluster_tile_idx < n_tiles_per_cluster;
        ...
        if (snrt_is_dm_core()) {
            snrt_dma_start_2d(
                local_itile, remote_ifmap + tile_idx * tile_offset,
                tile_seq_len * l.hidden_dim * data_type_size,
                tile_seq_len * l.hidden_dim * data_type_size,
                l.seq_len * l.hidden_dim * data_type_size, l.batch_size);
            snrt_dma_wait_all();
        }
        ...
        if (snrt_is_compute_core()) {
            ...
            l.funcptr((float *)local_itile, (float *)local_weight,
                      (float *)local_otile, l.batch_size, tile_seq_len,
                      l.hidden_dim, l.eps);
            snrt_mcycle();
        }
        ...
        if (snrt_is_dm_core()) {
            snrt_dma_start_2d(
                remote_ofmap + tile_idx * tile_offset, local_otile,
                tile_seq_len * l.hidden_dim * data_type_size,
                l.seq_len * l.hidden_dim * data_type_size,
                tile_seq_len * l.hidden_dim * data_type_size, l.batch_size);
            snrt_dma_wait_all();
        }
    }
