...
static inline void vexpf_baseline(double *a, double *b, uint32_t len,
                                  uint32_t batch_size) {
    ...
    for (int iteration = 0; iteration < n_iterations; iteration++) {
        ...
        if (snrt_cluster_core_idx() == 0) {
            ...
            if (iteration > 0 && iteration < n_iterations - 1) {
                ...
                // Loop over the batch size (processed 4 elements at a time)
                for (int i = 0; i < batch_size; i += 4) {
                    asm volatile(
                        ...
                        // clang-format on
                        : [in_addr] "+r"(comp_a_ptr), [out_addr] "+r"(
                                                          comp_b_ptr)
