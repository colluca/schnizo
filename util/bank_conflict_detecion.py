...
# One exception is in case there is only one LSU, and thus the dispatch is also delayed by one cycle.
...
# - The address that was accessed: The address is inside the [ and ] after the "<~~" or "~~>" part. The word infront of [ is arbitrary.
...
def extract_loads_and_stores(trace_file):
    ...
    with open(trace_file, 'r') as f:
        for line in f:
            match = re.search(
                r'^\d+\s+(\d+)\s+.*\s+(0x[0-9a-fA-F]+).*#;\s+(\S+)\s+.*[<~~|~~>]\s+\S+\[(0x[0-9a-fA-F]+)\]',
                line
            )
            if match:
...
def find_conflicts(trace_files, trace_file_prefix, num_banks, base_address, bank_width_bits):
    all_accesses = find_all_accesses(trace_files, trace_file_prefix, num_banks, base_address, bank_width_bits)
    ...
    for bank_id, bank_conflicts in all_conflicts.items():
        ...
        for cycle, conflicts in bank_conflicts.items():
            ...
            for detail in conflicts:
                print(f"    Core {detail['core_id']} accessed address {detail['address']} at PC {detail['pc']} ({detail['type']})")
        print("---------------------")
...
def generate_conflicts_report(all_conflicts):
    ...
    for bank_id, bank_conflicts in all_conflicts.items():
        ...
        for cycle, conflicts in bank_conflicts.items():
            ...
            for detail in conflicts:
                report.append(f"    Core {detail['core_id']} accessed address {detail['address']} at PC {detail['pc']} ({detail['type']})")
        report.append("---------------------")
    report.append("\n")
    return "\n".join(report)
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="File to save the conflicts report. If not specified, report will be printed to stdout."
    )
    ...
    trace_files = find_trace_files(args.traces_file_path, args.trace_file_prefix, args.trace_file_suffix)

    if not trace_files:
        print("No trace files found. Please check the path and prefix.")
        return

    print(f"Found {len(trace_files)} trace files in {args.traces_file_path} with prefix '{args.trace_file_prefix}'.")

    all_conflicts = find_conflicts(trace_files, args.trace_file_prefix, args.num_banks, args.base_address, args.bank_width)
