...
DASM_LINE_REGEX = (
    r"^\s*(?P<sim_time>\d+)\s+(?P<cycle>\d+)\s+(?P<priv_lvl>[A-Z])\s+"
    r"(?P<loop_state>[^\s]+)\s+#;\s+(?P<extra_str>.*?)(?=\s*$)"
)
    ...
    if (is_lsu):
        if (extras['lsu_is_store']):
            if extras['lsu_is_float']:
                register = REG_ABI_NAMES_F[rs2]
            else:
                register = REG_ABI_NAMES_I[rs2]
            fmt = LSU_SIZE_TO_FLOAT[extras['lsu_size']]
            if extras['lsu_is_float']:
                value_str = flt_lit(extras['lsu_store_data'], fmt)
            else:
                value_str = int_lit(extras['lsu_store_data'])
            addr_str = int_lit(extras['lsu_addr'], as_hex=True)
            comment.append(
                f"{register} = {value_str} ~~> "
                f"{LSU_SIZES[extras['lsu_size']]}[{addr_str}]"
            )
        elif (extras['lsu_is_load']):
            if extras['lsu_is_float']:
                register = REG_ABI_NAMES_F[rd]
            else:
                register = REG_ABI_NAMES_I[rd]
            addr_str = int_lit(extras['lsu_addr'], as_hex=True)
            comment.append(
                f"{register} <~~ {LSU_SIZES[extras['lsu_size']]}[{addr_str}]"
            )
    # CSR
    if ('fu_type' in extras):
...
def gen_rescap_trace(extras):
    ...
    fp_str = '(float)' if extras['rd_is_fp'] else ''
    result_iter = extras['result_iter']
    return (
        f"{'':<10} {'':<26}  {'':<6} #; RESCAP {producer} <-- "
        f"{result} {fp_str}, result iter = {result_iter}"
    )
...
def gen_dispatch_perfetto(sim_time, cycle, priv_lvl, loop_state, extras,
    ...
    perf_events.append(trace.start_instr(fu_str, instr_str,
    ...
    if (fu_str == FU_NONE):
        ...
        perf_events.append(trace.end_instr(fu_str, (cycle+1) * CLOCK_PERIOD_NS,
...
def main():
    ...
    with args.output as trace_file:
        ...
        # Make sure all instructions are offset to reflect their actual start times
    ...
    if args.perfetto_trace:
        with args.perfetto_trace as perfetto_file:
            perfetto_trace = trace.create(all_tracks, perf_events)
            trace.write_to_file(perfetto_trace, perfetto_file)
...
#     # If in single issue (LOOP_REGULAR, LOOP_HWLOOP, LOOP_LCP1 & LOOP_LCP2)
#     # combine dispatch & writeback onto one line.
#     # If there are more than 1 writebacks, add all successive writebacks on
#     # a separate line
#     if loop_states.issubset({LOOP_REGULAR, LOOP_HWLOOP, LOOP_LCP1, LOOP_LCP2}):
#         dispatch_event = next(
#             (event for event in events if event[4]['event'] == "dispatch"),
#             None
#         )
#         writeback_events = [
#             event for event in events if event[4]['event'] == "writeback"
#         ]
...
#             if writeback_events:
#                 writeback_traces = [
#                     gen_writeback_trace(writeback_event[4], 0)
