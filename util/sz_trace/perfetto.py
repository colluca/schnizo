...
Each functional unit (FU) is represented by a track where we distinguish between regular execution and FREP execution.
During FREP the instructions are mapped to the producer IDs.

Required data structures from external:
- all_tracks: A default dict holding all tracks. Indexed by the fu_string and/or the instruction count.
- inflight_tracks: A default dict holding a deque for each track/event which is currently inflight.
"""

# When generating the trace, also pass all in all events. This is a list of TracePacket objects.

ROOT_TRACK_NAME = "Schnizo Trace"
ROOT_TRACK_ID = 10

START_OFFSET_NAME = "Z Start offset (dummy to synchronize with cycle count)"
START_OFFSET_ID = 11
...
#     ALU_TYPE_NAME + "0.0" : ALU_TYPE_ID + 1 * FU_OFFSET + 1 * SLOT_OFFSET,  # ALU0.0
#     ALU_TYPE_NAME + "0.1" : ALU_TYPE_ID + 1 * FU_OFFSET + 2 * SLOT_OFFSET,  # ALU0.1
#     ALU_TYPE_NAME + "0.2" : ALU_TYPE_ID + 1 * FU_OFFSET + 3 * SLOT_OFFSET,  # ALU0.2
#     ALU_TYPE_NAME + "0.3" : ALU_TYPE_ID + 1 * FU_OFFSET + 4 * SLOT_OFFSET,  # ALU0.3
...
# The lower digits of the number are used to identify the FU Types, FU IDs and slots.
# A track for an instruction thus resembles like:
# 10000000 * instr_number + <FU type + FU ID + slot id>
...
# Returns the corresponding track event if it exists in the all_tracks dictionary. If the track
# does not exist, it creates a new one and links its to its parents.
...
def get_track(fu_string, all_tracks):
    ...
    if (fu_type not in FU_TYPE_OFFSET):
        raise ValueError(f"Invalid FU type: {fu_type}. Must be one of {list(FU_TYPE_OFFSET.keys())}")

    # Detect which type of track we want to create
    # First check the level (i.e., FU, FU ID or slot ID)
    # For FU type it is simple. Just create a new track.
    # For FU ID the FU type must exist. Otherwise we must create both.
    # For slot ID both FU type and FU ID must exist.

    # If it is only the fu_type, we can create the track directly.
    # We must explicitly check for None as the integer 0 is "true" in Python.
    if ((fu_type is not None) and (fu_id is None) and (slot_id is None)):
        new_track = perfetto_trace_pb2.TracePacket()
        new_track.track_descriptor.uuid = FU_TYPE_OFFSET[fu_type]
        new_track.track_descriptor.parent_uuid = ROOT_TRACK_ID
        new_track.track_descriptor.name = fu_type
        new_track.track_descriptor.child_ordering = perfetto_trace_pb2.TrackDescriptor.ChildTracksOrdering.LEXICOGRAPHIC
        all_tracks[fu_string] = new_track
        return all_tracks[fu_string]

    # If it is a fu_id, we must check if the fu_type exists
    if ((fu_type is not None) and (fu_id is not None) and (slot_id is None)):
        # Check if the fu_type track exists. If not create recursively
        fu_type_track = get_track(fu_type, all_tracks)

        # Create the new fu_id track. Offset the uuid by 1 offset because parent uses "0" offset.
        uuid = FU_TYPE_OFFSET[fu_type] + (fu_id + 1) * FU_ID_OFFSET
        new_track = perfetto_trace_pb2.TracePacket()
        new_track.track_descriptor.uuid = uuid
        new_track.track_descriptor.parent_uuid = fu_type_track.track_descriptor.uuid
        new_track.track_descriptor.name = fu_type + str(fu_id)
        new_track.track_descriptor.child_ordering = perfetto_trace_pb2.TrackDescriptor.ChildTracksOrdering.LEXICOGRAPHIC
        all_tracks[fu_string] = new_track
        return all_tracks[fu_string]

    # If it is a slot_id, we must check if the fu_type and fu_id exist
    if ((fu_type is not None) and (fu_id is not None) and (slot_id is not None)):
        # Check if the fu_id track exists. If not create recursively. This also checks if the
        # fu_type exists.
        fu_id_track = get_track(fu_type + str(fu_id), all_tracks)

        # Create the new slot_id track. Offset the uuid by 1 offset because parent uses "0" offset.
        uuid = FU_TYPE_OFFSET[fu_type] + (fu_id + 1) * FU_ID_OFFSET + (slot_id + 1) * SLOT_OFFSET
        new_track = perfetto_trace_pb2.TracePacket()
        new_track.track_descriptor.uuid = uuid
        new_track.track_descriptor.parent_uuid = fu_id_track.track_descriptor.uuid
        new_track.track_descriptor.name = fu_type + str(fu_id) + "." + str(slot_id)
        all_tracks[fu_string] = new_track
        return all_tracks[fu_string]

    raise ValueError(f"Invalid FU string format when getting track: {fu_string}")
...
# Perfetto always sets the time to 0 for the first event. We want consistency with the trace /
# we want the time to match the cycle count. Therefore, we add an artificial start events.
def add_start_offset(start_time_ns, all_tracks):
    # The root track must exist that we can link the offset track to it
    root_track = get_root_track(all_tracks)

    start_offset_track = perfetto_trace_pb2.TracePacket()
    start_offset_track.track_descriptor.uuid = START_OFFSET_ID
    start_offset_track.track_descriptor.name = START_OFFSET_NAME
    start_offset_track.track_descriptor.parent_uuid = root_track.track_descriptor.uuid
    all_tracks[START_OFFSET_NAME] = start_offset_track

    start_event = perfetto_trace_pb2.TracePacket()
    start_event.timestamp = 0
    start_event.track_event.type = perfetto_trace_pb2.TrackEvent.TYPE_SLICE_BEGIN
    start_event.track_event.track_uuid = START_OFFSET_ID
    start_event.track_event.name = START_OFFSET_NAME
    start_event.trusted_packet_sequence_id = TRUSTED_PKG_SEQ_ID

    end_event = perfetto_trace_pb2.TracePacket()
    end_event.timestamp = start_time_ns
    end_event.track_event.type = perfetto_trace_pb2.TrackEvent.TYPE_SLICE_END
    end_event.track_event.track_uuid = START_OFFSET_ID
    end_event.trusted_packet_sequence_id = TRUSTED_PKG_SEQ_ID

    return (start_event, end_event)


def create_trace(all_tracks, events):
...
def write_trace_to_file(trace, file):
