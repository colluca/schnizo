...
module schnizo_loop_controller import schnizo_pkg::*; #(
  ...
  assign new_loop = '{
    ...
    loop_state:      ((loop_iterations_i < 3'd3) || (frep_mode_i == FrepModeHwLoop)) ?
                      LoopHwLoop : LoopLcp1
  };
  ...
  always_comb begin
    ...
    unique case (loop_info_q.loop_state)
      ...
      LoopLcp1: begin
        ...
        if (dispatch_loop_end_instr || wait_for_retirement_q) begin
          if (all_rs_finish_i) begin
            ...
            // TODO(colluca): is this comment out of date?
        ...
        if (exit_frep) begin
          ...
          if (all_rs_finish_i) begin
            ...
            // wait for retirement
            // TODO(colluca): add separate state to wait for retirement before moving to LCP2,
            //                HW loop and LEP
      ...
      LoopLcp2: begin
        loop_jump_o = at_loop_end_instr && !current_loop_finish;
        // If the last instruction of LCP2 is a multicycle one, we go in a special waiting state.
