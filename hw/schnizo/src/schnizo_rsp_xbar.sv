...
module schnizo_rsp_xbar #(
  ...
  payload_t [NumOut-1:0][NumInp-1:0] mux_data;
  logic     [NumOut-1:0][NumInp-1:0] mux_valid;
  ...
  for (genvar i = 0; i < NumInp; i++) begin : gen_connection_demuxes
    for (genvar j = 0; j < NumOut; j++) begin : gen_connection_muxes
      assign mux_data[j][i] = demux_data[i][j];
      assign mux_valid[j][i] = demux_valid[i][j];
      assign demux_ready[i][j] = mux_ready[j][i];
    end
  end

  // Generate the output muxes
  for (genvar j = 0; j < NumOut; j++) begin : gen_muxes
