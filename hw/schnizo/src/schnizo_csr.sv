...
// Description: The CSR module which hosts all CSR, handles all CSR instructions,
...
module schnizo_csr import schnizo_pkg::*; #(
  ...
  // Interrupt codes
  localparam logic [30:0] IRQ_MSI = 3;
  localparam logic [30:0] IRQ_MTI = 7;
  localparam logic [30:0] IRQ_MEI = 11;
  localparam logic [30:0] IRQ_MCI = 19;
  localparam logic [30:0] IRQ_SSI = 1;
  localparam logic [30:0] IRQ_STI = 5;
  localparam logic [30:0] IRQ_SEI = 9;
  localparam logic [30:0] IRQ_SCI = 17;
  ...
  always_comb begin : csr_write_update
    ...
    if (interrupt_o) begin
      ...
      if      (meip) mcause_d.exception_code = IRQ_MEI;
      else if (mtip) mcause_d.exception_code = IRQ_MTI;
      else if (msip) mcause_d.exception_code = IRQ_MSI;
      else if (mcip) mcause_d.exception_code = IRQ_MCI;
      else if (seip) mcause_d.exception_code = IRQ_SEI;
      else if (stip) mcause_d.exception_code = IRQ_STI;
      else if (ssip) mcause_d.exception_code = IRQ_SSI;
      else if (scip) mcause_d.exception_code = IRQ_SCI;
    end
