...
// Description: Fully connected stream crossbar with selective request masking.
...
module schnizo_xbar #(
  ...
  for (genvar j = 0; unsigned'(j) < NumOut; j++) begin : gen_outs
    ...
    // requested iteration.
    payload_t [NumInp-1:0] mask_out;
    logic     [NumInp-1:0] mask_out_valid;
    logic     [NumInp-1:0] mask_out_ready;

    assign mask_out = out_data[j];

    for (genvar i = 0; unsigned'(i) < NumInp; i++) begin : gen_out_mask
      assign mask_out_valid[i] = (res_iter_i[j] == mask_out[i].requested_iter) && out_valid[j][i];
    end
    assign out_ready[j] = mask_out_ready;
    ...
    rr_arb_tree #(
      ...
      .flush_i,
      .rr_i    ( rr_i[j]        ),
      .req_i   ( mask_out_valid ),
      .gnt_o   ( mask_out_ready ),
      .data_i  ( mask_out       ),
      .req_o   ( arb_valid      ),
      .gnt_i   ( arb_ready      ),
      .data_o  ( arb.data       ),
