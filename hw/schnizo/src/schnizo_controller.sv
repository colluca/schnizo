...
module schnizo_controller import schnizo_pkg::*; #(
  ...
  // TODO(colluca): (!barrier_stall_i && !wfi_i) is reused in multiple places.
  //                Assign it to an intermediate signal, and comment on difference
  //                with other stall conditions.
  always_comb begin : pc_update
    pc_d = pc_q; // per default stay at current PC

    if (exception_o) begin
      pc_d = mtvec_i;
    end else if (!stall_o && !wfi_i && !barrier_stall_i) begin
      // If we don't stall, step the PC unless we are waiting for an event or are stalled by the
      // cluster hw barrier.
      if (mret_o) begin
        pc_d = mepc_i;
      end else if (sret_o) begin
        pc_d = sepc_i;
