...
//
// Single-issue dispatcher forwards instructions to the first functional unit of its kind.
// Reservation station dispatcher contains a dispatcher buffer for each functional unit and forwards instructions to
// all available functional units in superscalar execution mode.
module schnova_dispatcher import schnova_pkg::*; #(
  ...
  if (XFREPO) begin : gen_rs_dispatcher
    schnova_rs_dispatcher #(
      ...
      // If all the instruction buffers are empty
