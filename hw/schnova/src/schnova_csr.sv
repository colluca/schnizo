...
//                 (even if rd = x0).
module schnova_csr import schnova_pkg::*; #(
  ...
  // These are snitch / schnova specific due to custom CSRs.
  ...
  always_comb begin : csr_read
    ...
    if (csr_read_en) begin
      unique case (csr_addr.address)
        ...
        schnova_pkg::CsrFrepState: begin
    ...
    if (csr_write_en_int) begin
      unique case (csr_addr.address)
        riscv_instr::CSR_MSTATUS: begin
          ...
          //  - spp (used for virtual memory support. schnova does not support VM)
