...
package schnova_tracer_pkg;
  ...
  function automatic string format_dispatch_extras(dispatch_trace_t trace);
    string extras = "";
    ...
    extras = $sformatf("%s'%s':0x%02x, ", extras, "phy_rs1", trace.phy_rs1);
    extras = $sformatf("%s'%s':0x%02x, ", extras, "phy_rs2", trace.phy_rs2);
    extras = $sformatf("%s'%s':0x%02x, ", extras, "phy_rs3", trace.phy_rs3);
    extras = $sformatf("%s'%s':0x%02x, ", extras, "phy_rd", trace.phy_rd);
