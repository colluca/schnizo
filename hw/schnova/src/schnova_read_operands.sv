  ...
  always_comb begin
    for (int unsigned instr_idx = 0; instr_idx < PipeWidth; instr_idx++) begin
      ...
      if (instr_dec_i[instr_idx].use_pc_as_op_a) begin
        ...
        fu_data_o[instr_idx].use_operand_a = 1'b1;
        ...
        fu_data_o[instr_idx].use_operand_a = instr_dec_i[instr_idx].use_rs1;
        if (instr_idx == 0) begin
      ...
      if ((instr_dec_i[instr_idx].fu == schnova_pkg::ALU ||
          ...
          fu_data_o[instr_idx].use_operand_b = 1'b1;
          fu_data_o[instr_idx].operand_b[XLEN-1:0] = instr_dec_i[instr_idx].imm;
      end else begin
          fu_data_o[instr_idx].use_operand_b = instr_dec_i[instr_idx].use_rs2;
        if (instr_idx == 0) begin
      ...
      if (instr_dec_i[instr_idx].use_imm_as_rs3) begin
        if (instr_idx == 0) begin
        fu_data_o[instr_idx].use_imm = 1'b1;
        ...
        //                with an assertion.
        if (instr_dec_i[instr_idx].use_imm          ||
            (instr_dec_i[instr_idx].use_imm_as_op_b &&
            !fu_data_o[instr_idx].use_operand_b)) begin
          fu_data_o[instr_idx].use_imm = 1'b1;
        end
