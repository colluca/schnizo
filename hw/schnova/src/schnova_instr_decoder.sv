...
module schnova_instr_decoder import schnova_pkg::*; #(
  ...
  always_comb begin
    ...
    instr_dec_o.fu = schnova_pkg::NONE;
    instr_dec_o.alu_op = schnova_pkg::AluOpAdd;
    instr_dec_o.lsu_op = schnova_pkg::LsuOpLoad;
    instr_dec_o.csr_op = schnova_pkg::CsrOpNone;
    instr_dec_o.fpu_op = schnova_pkg::FpuOpFadd;
    ...
    instr_dec_o.frep_mode     = schnova_pkg::frep_mode_e'(1'b0);
    ...
    unique case (instr.rtype.opcode)
      ...
      // --------------------------------
      OpcodeOpImm: begin
        instr_dec_o.fu = schnova_pkg::ALU;
        imm_select = IIMM;
        instr_dec_o.rs1 = instr.itype.rs1;
        instr_dec_o.rd = instr.itype.rd;
        unique case (instr.itype.funct3)
          3'b000: instr_dec_o.alu_op = schnova_pkg::AluOpAdd;  // ADDI
          3'b010: instr_dec_o.alu_op = schnova_pkg::AluOpSlt;  // SLTI
          3'b011: instr_dec_o.alu_op = schnova_pkg::AluOpSltu; // SLTIU
          3'b100: instr_dec_o.alu_op = schnova_pkg::AluOpXor;  // XORI
          3'b110: instr_dec_o.alu_op = schnova_pkg::AluOpOr;   // ORI
          3'b111: instr_dec_o.alu_op = schnova_pkg::AluOpAnd;  // ANDI

          3'b001: begin
            instr_dec_o.alu_op = schnova_pkg::AluOpSll; // SLLI
            if (instr.instr[31:25] != 7'b0) illegal_instr = 1'b1;
          end
          3'b101: begin
            if (instr.instr[31:25] == 7'b0) begin
              instr_dec_o.alu_op = schnova_pkg::AluOpSrl; // SRLI
            end else if (instr.instr[31:25] == 7'b010_0000) begin
              instr_dec_o.alu_op = schnova_pkg::AluOpSra; // SRAI
            end else begin
              illegal_instr = 1'b1;
            end
          end
      ...
      OpcodeOpReg: begin
        ...
        unique case ({instr.rtype.funct7, instr.rtype.funct3})
          {7'b000_0000, 3'b000} : instr_dec_o.alu_op = schnova_pkg::AluOpAdd; //Add
          {7'b010_0000, 3'b000} : instr_dec_o.alu_op = schnova_pkg::AluOpSub; //Sub
          {7'b000_0000, 3'b010} : instr_dec_o.alu_op = schnova_pkg::AluOpSlt; //Set Lower Than
          {7'b000_0000, 3'b011} : instr_dec_o.alu_op = schnova_pkg::AluOpSltu;//Set Lower Than Uns.
          {7'b000_0000, 3'b100} : instr_dec_o.alu_op = schnova_pkg::AluOpXor; //Xor
          {7'b000_0000, 3'b110} : instr_dec_o.alu_op = schnova_pkg::AluOpOr;  //Or
          {7'b000_0000, 3'b111} : instr_dec_o.alu_op = schnova_pkg::AluOpAnd; //And
          {7'b000_0000, 3'b001} : instr_dec_o.alu_op = schnova_pkg::AluOpSll; //Shift Left Logical
          {7'b000_0000, 3'b101} : instr_dec_o.alu_op = schnova_pkg::AluOpSrl; //Shift Right Logical
          {7'b010_0000, 3'b101} : instr_dec_o.alu_op = schnova_pkg::AluOpSra; //Shift Right Arithm
          {7'b000_0001, 3'b000} : begin
            instr_dec_o.fu = schnova_pkg::MUL;
            instr_dec_o.alu_op = schnova_pkg::AluOpMul;
          end
          {7'b000_0001, 3'b001} : begin
            instr_dec_o.fu = schnova_pkg::MUL;
            instr_dec_o.alu_op = schnova_pkg::AluOpMulh;
          end
          {7'b000_0001, 3'b010} : begin
            instr_dec_o.fu = schnova_pkg::MUL;
            instr_dec_o.alu_op = schnova_pkg::AluOpMulhsu;
          end
          {7'b000_0001, 3'b011} : begin
            instr_dec_o.fu = schnova_pkg::MUL;
            instr_dec_o.alu_op = schnova_pkg::AluOpMulhu;
          end
          ...
          {7'b000_0001, 3'b111}: begin // REMU
            instr_dec_o.fu = schnova_pkg::MULDIV;
          end
          default: begin
            illegal_instr = 1'b1;
          end
      ...
      // --------------------------------
      OpcodeStore: begin
        instr_dec_o.fu = schnova_pkg::STORE;
        imm_select = SIMM;
        instr_dec_o.rs1 = instr.stype.rs1;
        instr_dec_o.rs2 = instr.stype.rs2;
        instr_dec_o.lsu_op = schnova_pkg::LsuOpStore;
      ...
      OpcodeLoad: begin
        instr_dec_o.fu = schnova_pkg::LOAD;
        imm_select = IIMM;
        instr_dec_o.rs1 = instr.itype.rs1;
        instr_dec_o.rd = instr.itype.rd;
        instr_dec_o.lsu_op = schnova_pkg::LsuOpLoad;
        ...
        unique case (instr.itype.funct3)
          ...
          3'b010: ; // LW
          3'b100,   // LBU
          3'b101: instr_dec_o.lsu_op = schnova_pkg::LsuOpLoadUnsigned; // LHU
          3'b110,
      ...
      OpcodeStoreFp: begin // STORE-FP
        ...
        // determine store size
        instr_dec_o.lsu_op = schnova_pkg::LsuOpFpStore;
        instr_dec_o.lsu_size = lsu_size_e'(instr.stype.funct3[13:12]);
        unique case (instr.stype.funct3)
      ...
      OpcodeLoadFp: begin // LOAD-FP
        ...
        // determine load size
        instr_dec_o.lsu_op = schnova_pkg::LsuOpFpLoad;
        instr_dec_o.lsu_size = lsu_size_e'(instr.itype.funct3[13:12]);
        unique case (instr.itype.funct3)
          // Only process instruction if corresponding extension is active (static)
      ...
      OpcodeMadd, OpcodeMsub, OpcodeNmsub, OpcodeNmadd: begin
        if (RVF || RVD) begin
          instr_dec_o.fu             = schnova_pkg::FPU;
          ...
          unique case (instr.r4type.opcode)
            7'b1000011: instr_dec_o.fpu_op = schnova_pkg::FpuOpFmadd; // FMADD
            7'b1000111: instr_dec_o.fpu_op = schnova_pkg::FpuOpFmsub; // FMSUB
            7'b1001011: instr_dec_o.fpu_op = schnova_pkg::FpuOpFnmsub; // FNMSUB
            7'b1001111: instr_dec_o.fpu_op = schnova_pkg::FpuOpFnmadd; // FNMADD
            default: illegal_instr = 1'b1;
          endcase
      ...
      OpcodeOpFp: begin
        if (RVF || RVD) begin
          ...
          unique case (instr.rftype.funct5)
            5'b00000: begin
              instr_dec_o.fpu_op = schnova_pkg::FpuOpFadd; // FADD_S, FADD_D
            end
            5'b00001: begin
              instr_dec_o.fpu_op = schnova_pkg::FpuOpFsub; // FSUB_S, FSUB_D
            end
            5'b00010: begin
              instr_dec_o.fpu_op = schnova_pkg::FpuOpFmul; // FMUL_S, FMUL_D
            end
            5'b00011: begin
              instr_dec_o.fpu_op = schnova_pkg::FpuOpFdiv; // FDIV_S, FDIV_D
            end
            5'b01011: begin
              instr_dec_o.fpu_op = schnova_pkg::FpuOpFsqrt; // FSQRT_S, FSQRT_D
            ...
            5'b00100: begin
              ...
              check_fpround_mode = 1'b0;
              instr_dec_o.fpu_op = schnova_pkg::FpuOpFsgnj; // we only use op_mode zero (NaN boxed)
            ...
            5'b00101: begin
              ...
              // Disable round mode check
              check_fpround_mode = 1'b0;
              instr_dec_o.fpu_op = schnova_pkg::FpuOpFminmax;
              unique case (instr.rftype.rm)
            ...
            5'b01000: begin
              ...
              unique case ({instr.rftype.fmt, instr.rftype.rs2})
                7'b01_00000: begin
                  instr_dec_o.fpu_op = schnova_pkg::FpuOpF2F; // FCVT_D_S
                  instr_dec_o.fpu_fmt_src = fpnew_pkg::FP32;
                end
                7'b00_00001: begin
                  instr_dec_o.fpu_op = schnova_pkg::FpuOpF2F; // FCVT_S_D
            ...
            5'b11000: begin
              ...
              unique case (instr.rftype.rs2)
                5'b00000: instr_dec_o.fpu_op = schnova_pkg::FpuOpF2I;        //FCVT_W_S, FCVT_W_D
                5'b00001: instr_dec_o.fpu_op = schnova_pkg::FpuOpF2Iunsigned;//FCVT_WU_S, FCVT_WU_D
                default: illegal_instr = 1'b1;
              endcase
            end
            5'b10100: begin
              check_fpround_mode = 1'b0;
              instr_dec_o.rd_is_fp = 1'b0;
              instr_dec_o.fpu_op = schnova_pkg::FpuOpFcmp;
              unique case (instr.rftype.rm)
            ...
            5'b11100: begin
              ...
              unique case (instr.rftype.rm)
                3'b000: begin
                  ...
                  instr_dec_o.fpu_op = schnova_pkg::FpuOpFsgnjSignExt; // FMV_X_W
                  instr_dec_o.fpu_rnd_mode = fpnew_pkg::RUP; // passthrough
                end
                3'b001: begin
                  instr_dec_o.rd_is_fp = 1'b0;
                  instr_dec_o.fpu_op = schnova_pkg::FpuOpFclassify; // FCLASS_S, FCLASS_D
                  instr_dec_o.fpu_rnd_mode = fpnew_pkg::RNE;
                end
                default: illegal_instr = 1'b1;
              endcase
            ...
            5'b11010: begin
              ...
              unique case (instr.rftype.rs2)
                5'b00000: instr_dec_o.fpu_op = schnova_pkg::FpuOpI2F;        //FCVT_S_W, FCVT_D_W
                5'b00001: instr_dec_o.fpu_op = schnova_pkg::FpuOpI2Funsigned;//FCVT_S_WU, FCVT_D_WU
                default: illegal_instr = 1'b1;
              endcase
            end
            5'b11110: begin
              ...
              instr_dec_o.rs1_is_fp  = 1'b0;
                instr_dec_o.fpu_op = schnova_pkg::FpuOpFsgnj; // FMV_W_X
                instr_dec_o.fpu_rnd_mode = fpnew_pkg::RUP; // passthrough
              end else begin
                illegal_instr = 1'b1;
              end
      ...
      // --------------------------------
      OpcodeBranch: begin
        instr_dec_o.fu        = schnova_pkg::CTRL_FLOW;
        imm_select            = SBIMM;
        instr_dec_o.rs1       = instr.stype.rs1;
        instr_dec_o.rs2       = instr.stype.rs2;
        ...
        case (instr.stype.funct3)
          3'b000: instr_dec_o.alu_op = schnova_pkg::AluOpEq;  // BEQ
          3'b001: instr_dec_o.alu_op = schnova_pkg::AluOpNeq; // BNE
          3'b100: instr_dec_o.alu_op = schnova_pkg::AluOpLt;  // BLT
          3'b101: instr_dec_o.alu_op = schnova_pkg::AluOpGe;  // BGE
          3'b110: instr_dec_o.alu_op = schnova_pkg::AluOpLtu; // BLTU
          3'b111: instr_dec_o.alu_op = schnova_pkg::AluOpGeu; // BGEU
          default: begin
            illegal_instr           = 1'b1;
            instr_dec_o.is_branch    = 1'b0;
          end
      ...
      // --------------------------------
      OpcodeSystem: begin
        instr_dec_o.fu = schnova_pkg::CSR;
        instr_dec_o.rd = instr.itype.rd;
        instr_dec_o.rs1 = instr.itype.rs1;
        case (instr.itype.funct3)
          ...
          3'b001: begin
            ...
            imm_select = IIMM;
            if (instr.itype.rd == '0) instr_dec_o.csr_op = schnova_pkg::CsrOpWrite;
            else instr_dec_o.csr_op = schnova_pkg::CsrOpSwap;
          end
          3'b010: begin
            // CSRRS: atomically Read and set Bits in the CSR based on rs1. Write to rd.
            //        If rs1 = x0, then do not write to CSR, just read.
            imm_select = IIMM;
            if (instr.itype.rs1 == '0) instr_dec_o.csr_op = schnova_pkg::CsrOpRead;
            else instr_dec_o.csr_op = schnova_pkg::CsrOpSet;
          end
          3'b011: begin
            ...
            imm_select = IIMM;
            if (instr.itype.rs1 == '0) instr_dec_o.csr_op = schnova_pkg::CsrOpRead;
            else instr_dec_o.csr_op = schnova_pkg::CsrOpClear;
          end
          3'b101: begin
        ...
        if (Xdma) begin
          unique case (instr.rtype.funct3)
            3'b000: begin // DMA instructions
              instr_dec_o.fu  = schnova_pkg::DMA;
              instr_dec_o.rd  = instr.rtype.rd;
              instr_dec_o.rs1 = instr.rtype.rs1;
              instr_dec_o.rs2 = instr.rtype.rs2;
              // Check fixed bits
            ...
            3'b010: begin // SSR instructions
              // The schnova does not feature SSRs
              illegal_instr = 1'b1;
            end
            default: illegal_instr = 1'b1;
          endcase
      ...
      OpcodeAmo: begin
        ...
        unique case (instr.atype.funct5)
          5'b00010: begin // LR_W
            instr_dec_o.lsu_op = schnova_pkg::LsuOpAmoLr;
            if (instr.atype.rs2 != '0) illegal_instr = 1'b1;
          end
          5'b00011: begin // SC_W
            instr_dec_o.lsu_op = schnova_pkg::LsuOpAmoSc;
          end
          5'b00001: begin // AMOSWAP_w
            instr_dec_o.lsu_op = schnova_pkg::LsuOpAmoSwap;
          end
          5'b00000: begin // AMOADD_W
            instr_dec_o.lsu_op = schnova_pkg::LsuOpAmoAdd;
          end
          5'b00100: begin // AMOXOR_W
            instr_dec_o.lsu_op = schnova_pkg::LsuOpAmoXor;
          end
          5'b01100: begin // AMOAND_W
            instr_dec_o.lsu_op = schnova_pkg::LsuOpAmoAnd;
          end
          5'b01000: begin // AMOOR_W
            instr_dec_o.lsu_op = schnova_pkg::LsuOpAmoOr;
          end
          5'b10000: begin // AMOMIN_W
            instr_dec_o.lsu_op = schnova_pkg::LsuOpAmoMin;
          end
          5'b10100: begin // AMOMAX_W
            instr_dec_o.lsu_op = schnova_pkg::LsuOpAmoMax;
          end
          5'b11000: begin // AMOMINU_W
            instr_dec_o.lsu_op = schnova_pkg::LsuOpAmoMinU;
          end
          5'b11100: begin // AMOMAXU_W
            instr_dec_o.lsu_op = schnova_pkg::LsuOpAmoMaxU;
          end
          default: illegal_instr = 1'b1;
        endcase
      end
